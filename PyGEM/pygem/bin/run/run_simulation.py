...
from pygem.oggm_compat import (
    ...
    single_flowline_glacier_directory_custom,
)
...
def getparser():
    ...
    parser.add_argument(
        ...
        help='Ratio of ddfsnow to ddfice (ddfice = ddfsnow / ddfsnow_iceratio)',
    )
    parser.add_argument(
        '-oggm_working_dir',
        action='store',
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='override root path for output files (default: use config root)',
    )
    # flags
    parser.add_argument(
        '-export_all_simiters',
...
def run(list_packed_vars):
    ...
    for glac in range(main_glac_rgi.shape[0]):
        ...
        try:
            ...
            # Special handling for Dixon Glacier - use custom geometry
            if glacier_str == '1.20947':
                print(f"Using custom loader for Dixon Glacier")
                gdir = single_flowline_glacier_directory_custom(glacier_str)
                gdir.is_tidewater = False
                calving_k = None
            elif glacier_rgi_table['TermType'] not in [1, 5] or not pygem_prms['setup']['include_frontalablation']:
            ...
            if (fls is not None) and (glacier_area_km2.sum() > 0):
                ...
                if args.option_calibration:
                    ...
                    modelprms_all = {
                        ...
                        'ddfice': [args.ddfsnow / args.ddfsnow_iceratio],
                ...
                for n_iter in range(nsims):
                    ...
                    ######################################
                        ...
                        ev_model = MassRedistributionCurveModel(
                            ...
                            water_level=water_level,
                            # Disable boundary check for elevation-band flowlines where
                            # bands are sorted terminus-to-head (thick[-1] is accumulation zone)
